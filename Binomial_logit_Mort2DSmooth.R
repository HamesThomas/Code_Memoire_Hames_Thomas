Mort2Dsmooth_Binomial_logit = function (x, y, Z, offset, W, overdispersion = FALSE, ndx = c(floor(length(x)/5), 
                             ...
                             df = NULL, method = 1, coefstart = NULL, control = list()) 
{
  ...
  check <- Mort2Dsmooth_checker_logit(x = x, y = y, Z = Z, offset = offset, 
  ...
  xmax <- xr + 0.01 * (xr - xl)
  xmin <- xl - 0.01 * (xr - xl)
  Bx <- MortSmooth_bbase_logit(x, xmin, xmax, ndx[1], deg[1])
  nbx <- ncol(Bx)
  yl <- min(y)
  yr <- max(y)
  ymax <- yr + 0.01 * (yr - yl)
  ymin <- yl - 0.01 * (yr - yl)
  By <- MortSmooth_bbase_logit(y, ymin, ymax, ndx[2], deg[2])
  nby <- ncol(By)
  ...
  if (is.null(a.init)) {
    ...
    a.init <- MortSmooth_BcoefB_logit(BBx, BBy, eta0)
  }
  else {
    a.init = a.init
  }
  ...
  if (MET == 1 | MET == 2) {
    if (over) {
      ...
      while (tol.over > 0.001 && i.over < 5) {
        i.over <- i.over + 1
        lambdas.hat <- Mort2Dsmooth_optimize_logit(x = x, y = y, 
        ...
        FIT <- Mort2Dsmooth_estimate_logit(x = x, y = y, Z = Z, 
                                     ...
                                     MAX.IT = MAX.IT)
        psi2.old <- psi2
        psi2 <- FIT$dev/(m * n - FIT$df)       ### Expression de l'overdispersion
  ...
  if (MET == 3) 
    {
    lambdas.hat <- lambdas
    FIT <- Mort2Dsmooth_estimate_logit(x = x, y = y, Z = Z, offset = offset, 
  ...
  if (MET == 4) 
    {
    Mort2Dsmooth_opt_df_logit <- function(X) {
      FIT <- Mort2Dsmooth_estimate_logit(x = x, y = y, Z = Z, 
                                   offset = offset, wei = wei, psi2 = psi2, Bx = Bx, 
                                   By = By, nbx = nbx, nby = nby, RTBx = RTBx, RTBy = RTBy, 
    ...
    lambdas.hat <- cleversearch_logit(fn = Mort2Dsmooth_opt_df_logit, 
  ...
  h <- FIT$h
  tolerance <- FIT$tol
  eta.hat <- matrix(MortSmooth_BcoefB_logit(Bx, By, coef), m, n, 
                    dimnames = list(x, y))
  fitted.values <- matrix(exp(offset + eta.hat)/(1 + exp(offset + eta.hat)),   ## Changed here also
  ...
  object <- list(call = call, m = m, n = n, tolerance = tolerance, 
                 ...
                 logit.qx = eta.hat)
  class(object) <- "Mort2Dsmooth.binomial.logit"
  object
}
...
MortSmooth_bbase_logit = function (x, xl, xr, ndx, deg) 
{
  dx <- (xr - xl)/ndx
...
MortSmooth_BcoefB_logit = function (X1, X2, mat) 
{
  BcoefB <- X1 %*% mat %*% t(X2)
  return(BcoefB)
}
...
Mort2Dsmooth_optimize_logit = function (x, y, Z, offset, wei, psi2, Bx, By, nbx, nby, RTBx, 
                                  RTBy, Px, Py, a.init, MON, TOL1, TOL2, RANGEx, RANGEy, MAX.IT, 
                                  MET) 
{
  
  Mort2Dsmooth_opt_ic_logit <- function(X) {
    FIT <- Mort2Dsmooth_estimate_logit(x = x, y = y, Z = Z, 
  ...
  lambdas.hat0 <- cleversearch_logit(fn = Mort2Dsmooth_opt_ic_logit, lower = c(log10(RANGEx[1]), 
  ...
  lambdas.hat <- cleversearch_logit(fn = Mort2Dsmooth_opt_ic_logit, lower = c(min.l.x, 
...
Mort2Dsmooth_estimate_logit = function (x, y, Z, offset, psi2, wei, Bx, By, nbx, nby, RTBx, 
                                  RTBy, lambdas, Px, Py, a.init, MON, TOL1, MAX.IT) 
{
  ...
  while (tol > TOL1 && i < MAX.IT) {
    i <- i + 1
    a <- Mort2Dsmooth_update_logit(x = x, y = y, Z = Z, offset = offset, 
                             psi2 = psi2, wei = wei, Bx = Bx, By = By, nbx = nbx, 
...
Mort2Dsmooth_update_logit = function (x, y, Z, offset, psi2, wei, Bx, By, nbx, nby, RTBx, 
                                RTBy, P, a) 
{
...
MortSmooth_BWB_logit = function (RTBx, RTBy, nbx, nby, W) 
{
  BWB <- t(RTBx) %*% W %*% RTBy
...
cleversearch_logit= function (fn, lower, upper, ngrid, startvalue, logscale = TRUE, 
